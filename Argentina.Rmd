  ...
  ## Dataset Provincias Argentinas
    {
      casos_pronvinciales <- read_excel("data/covid_departamental.xlsx")
       ...
       names(casos_pronvinciales) <- c("provincia", "departamento",  "localidad","fecha","cantidad", "recuperados", "fallecidos", "internados", "uti", "enestudio", "aislados", "estudiados", "descartados")
...
```{r mapaActivos, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',  out.height='100%'}
 activos <- casos_pronvinciales %>% filter(departamento=="TOTAL GENERAL") %>%       mutate(activos=cantidad-recuperados-fallecidos) %>%  
  ...
  activos$provincia <- if_else(activos$provincia =="Ciudad Autónoma de Buenos Aires",
                               "Ciudad de Buenos Aires", activos$provincia)
...
```{r dptoArg, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  casos_pronvincialesT <- casos_pronvinciales %>% select(provincia, localidad, cantidad, fallecidos, recuperados)
  ...
  casos_pronvincialesT <- casos_pronvincialesT %>%
    ...
    select(provincia, departamen, casos, fallecidos, recuperados)
  names(casos_pronvincialesT) <- c("provincia", "departamento", "casos", "fallecidos", "recuperados")
  ...
  deptos_covid$recuperados<- if_else(is.na(deptos_covid$recuperados), 0, deptos_covid$recuperados)
  deptos_covid$fallecidos<- if_else(is.na(deptos_covid$fallecidos), 0, deptos_covid$fallecidos)
  ...
  leaflet(data = deptos_covid) %>% addTiles() %>%
    ...
    addPolygons( color = "#444444", weight = 1,
                 ...
                 label = paste(deptos_covid$provincia, " - ",
                               deptos_covid$departamen, " <br/> Confirmados:", 
                               deptos_covid$casos,  "<br/> Recuperados: ",
                               deptos_covid$recuperados,  "<br/> Fallecidos: ",
                               deptos_covid$fallecidos),
                 popup = paste(deptos_covid$provincia, " - ",
                               deptos_covid$departamen, " <br/> Confirmados:", 
                               deptos_covid$casos,  "<br/> Recuperados: ",
                               deptos_covid$recuperados,  "<br/> Fallecidos: ",
                               deptos_covid$fallecidos),
...
```{r dptoAMBA, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  amba <- casos_pronvinciales %>% filter(localidad %in% amba_loc$departamento ) %>% select(provincia, localidad, cantidad, fallecidos, recuperados)
  ...
  amba$departamen <- if_else(is.na(amba$departamen)&
                               amba$provincia=="Ciudad Autónoma de Buenos Aires" &
                               amba$localidad=="TOTAL GENERAL", "Ciudad Autónoma de Buenos Aires",
                             amba$departamen)
  ...
  # names(deptos_covid) <- c("id", "departam", "provincia", 
  #                          "cantidad", "fallecidos", "recuperados", "geometry")
